...
function findImage(files: any[], keywords: string[], excludeKeywords: string[] = []) {
  const found = files.find((f) => {
    const pathLower = f.filePath.toLowerCase();
    const nameLower = f.name.toLowerCase();
    const tagsLower = new Set((f.tags ?? []).map((t: string) => t.toLowerCase()));

    const matchesKeyword = keywords.some(
      (k) => pathLower.includes(k) || nameLower.includes(k) || tagsLower.has(k),
    );
...
function Index() {
  ...
  const resolvedImages = useMemo(() => {
    ...
    return {
      gettingReady: findImage(files, ["getting ready", "getting-ready", "morning"]) || {
        url: FALLBACK_CHAPTER_IMAGES.gettingReady,
        w: 1600,
        h: 1067,
      },
      ceremony: findImage(files, ["ceremony"]) || {
        url: FALLBACK_CHAPTER_IMAGES.ceremony,
        w: 1920,
        h: 1280,
      },
      reception: findImage(files, ["reception"], ["gifts", "bridal"]) || {
        url: FALLBACK_CHAPTER_IMAGES.reception,
        w: 1920,
        h: 1080,
      },
      gifts: findImage(files, ["gifts"]) || {
        url: FALLBACK_CHAPTER_IMAGES.gifts,
        w: 1600,
        h: 1067,
      },
      party: findImage(files, ["party", "dance"]) || {
        url: FALLBACK_CHAPTER_IMAGES.party,
        w: 1920,
        h: 1280,
      },
      families: findImage(files, ["families", "family"]) || {
  ...
  return (
    <main className="bg-ivory text-charcoal">
      ...
      <Chapter
        ...
        eyebrow="The Church"
        title="“I do.”"
        body="At The Ministry of Repentance and Holiness, before God and the people who raised them, they spoke the words. Light through coloured glass. A held breath. A whole life, said out loud."
        meta="The Ministry of Repentance and Holiness, Ahero Altar"
